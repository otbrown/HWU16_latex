...
\begin{itemize}
\item \textbackslash\ identifies a command sequence (\textbackslash textbackslash typesets a \textbackslash).
\item \textbackslash\ is also an escape sequence for typesetting symbols like \{, \}, and \&.
\item \textbackslash\textbackslash\ \emph{requests} a newline.
\item \textbackslash emph\{\} for \emph{italics}, \textbackslash textbf\{\} for \textbf{bold}.
\item \textbackslash section\{Section Title\} and \textbackslash subsection\{Subsection Title\} creates sections and subsections which are automatically labelled and included in your table of contents.
\item \textbackslash label\{\} can be used to label sections, figures, equations, etc. and is \emph{highly} recommended. Especially in conjunction with the cleveref and hyperref packages.
\end{itemize}
\end{frame}
...
Inline maths:\\
\quad \textbackslash( E = \textbackslash frac\{1\}\{2\} mv\^{}\{2\} \textbackslash) \\
\vspace{\baselineskip}
\quad One of Einstein's discoveries was the equation \(E = \frac{1}{2}mv^{2}\), equating energy and mass. \\
\vspace{\baselineskip}
Display maths:\\
\quad \textbackslash begin\{equation\} \\
\quad \quad \textbackslash rho = \textbar \textbackslash psi \textbackslash rangle \textbackslash langle \textbackslash psi \textbar \\
\quad \quad \textbackslash label\{eq:1\} \\
\quad \textbackslash end\{equation\} \\
\begin{equation}
...
\frametitle{Basics}
\framesubtitle{BibTeX}
Then you need to include a \textbackslash bibliographystyle\{\} command, and \textbackslash bibliography\{your-bib-file\} in the document:
\begin{lstlisting}[label={lst:6}]
\bibliographystyle{acm}
\bibliography{intro_latex}
\end{lstlisting}

You then simply use the \textbackslash cite\{citation-label\} command to reference things like this conference paper I wrote \cite{Bea13}, or the book \emph{Numerical Recipes} \cite{NR}.
\end{frame}
